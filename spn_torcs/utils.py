...
def get_guide_action(bin_divide, action, lb=-1.0, ub=1.0):
    ...
    action = np.clip(((action - lb) / (ub - lb) * _bin_divide), lb, ub-0.001).astype(np.uint8)
...
def sample_action_new(args, net, obs_var, action_var, guides, info=None):
    begin_time = time.time()
    obs_var = (obs_var / 255.0).repeat(args.min_branch_width, 1, 1, 1)
    with torch.no_grad():
        latent, hidden = net(obs_var, function='extract_feature')
    action_var = action_var.repeat(args.min_branch_width, 1, 1)
    latent = tile_first(latent, action_var)
    current_loss = np.zeros(args.min_branch_width, dtype=np.float32)

    for step in range(args.pred_step):
        consistency_mask = generate_consistency_mask(args, action_var[:, -1, :], guides)
        with torch.no_grad():
            p = net(latent[-1], function='get_p')
            p = F.softmax(p / args.temperature, dim=1) * consistency_mask
            p /= torch.sum(p, dim=1).view(-1, 1)
            p = p.data.cpu().numpy()
        actions = multiple_sample(args, p, guides)

        latent, hidden, action_var, current_loss = clone_feature(args, latent, hidden, action_var, current_loss)
        with torch.no_grad():
            data, latent, hidden = net(latent, actions, hidden, function='one_step')
        action_var = update_action_var(action_var, actions)

        loss = get_loss(args, data)
        current_loss += loss.data.cpu().numpy().reshape(-1)
        latent, hidden, action_var, current_loss = prune(args, latent, hidden, action_var, current_loss)
    idx = np.argmin(current_loss)
    print('Sampling takes %0.2f second.' % (time.time() - begin_time))
    for i in range(args.pred_step):
        print(action_var[idx, args.frame_history_len-1+i].data.cpu().numpy())
    return action_var[idx, args.frame_history_len-1].data.cpu().numpy()


def generate_consistency_mask(args, last_action, guides):
    last_action = last_action.data.cpu().numpy()
    batch_size, action_dim = last_action.shape
    assert action_dim == args.num_total_act
    masks = []
    for i in range(batch_size):
        guide = guides[get_guide_action(args.bin_divide, last_action[i])].reshape(1, -1)
        mask = np.exp(-np.linalg.norm(guides - guide, axis=1) * args.consistency_factor)
...
def multiple_sample(args, p, guides, lb=-1.0, ub=1.0):
    res = []
    full_range = ub - lb
    classes = p.shape[1]
    for i in range(args.branch_multiplier):
        for j in range(args.min_branch_width):
            c = np.random.choice(range(classes), p=p[j])
            res.append(np.array(guides[c]) + np.array(list(map(lambda x: np.random.uniform(low=-full_range / 2.0 / x, high=full_range / 2.0 / x), args.bin_divide))))
    res = list(map(lambda x: x.reshape(1, -1), res))
    res = torch.from_numpy(np.concatenate(res, axis=0)).float()
    if torch.cuda.is_available():
        res = res.cuda()
    return res


def clone_feature(args, latent, hidden, action_var, current_loss):
    for i in range(args.frame_history_len):
        for j in range(len(latent[i])):
            latent[i][j] = latent[i][j].repeat(args.branch_multiplier, 1, 1, 1)

    hidden = hidden.repeat(args.branch_multiplier, 1, 1, 1)
    action_var = action_var.repeat(args.branch_multiplier, 1, 1)
    current_loss = np.repeat(current_loss, args.branch_multiplier, axis=0)

    return latent, hidden, action_var, current_loss


def update_action_var(action_var, actions):
    actions = actions.unsqueeze(dim=1)
    return torch.cat([action_var, actions], dim=1)


def get_loss(args, output):
    loss = 0
    if args.sample_with_collision:
        output['coll_prob'] = F.softmax(output['coll_prob'], -1)
        if 'torcs' in args.env or 'gta' in args.env:
            coll_ls = -torch.round(output['coll_prob'][:, 0]) * output['dist'].view(-1) + torch.round(output['coll_prob'][:, 1]) * 20.0
        elif 'carla' in args.env:
            coll_ls = -torch.round(output['coll_prob'][:, 0]) * output['speed'].view(-1) + torch.round(output['coll_prob'][:, 1]) * 10.0
        loss += coll_ls
    if args.sample_with_offroad:
        output['offroad_prob'] = F.softmax(output['offroad_prob'], -1)
        if 'torcs' in args.env or 'gta' in args.env:
            off_ls = -torch.round(output['offroad_prob'][:, 0]) * output['dist'].view(-1) + torch.round(output['offroad_prob'][:, 1]) * 20.0
        elif 'carla' in args.env:
            off_ls = -torch.round(output['offroad_prob'][:, 0]) * output['speed'].view(-1) + torch.round(output['offroad_prob'][:, 1]) * 10.0
        loss += off_ls
    return loss


def prune(args, latent, hidden, action_var, current_loss):
    _idx = np.argsort(current_loss, axis=0)[:args.min_branch_width]
    idx = torch.from_numpy(_idx)
    if torch.cuda.is_available():
        idx = idx.cuda()
    for i in range(args.frame_history_len):
        for j in range(len(latent[i])):
            latent[i][j] = latent[i][j][idx]
    hidden = hidden[idx]
    action_var = action_var[idx]
    current_loss = current_loss[_idx]
    return latent, hidden, action_var, current_loss
